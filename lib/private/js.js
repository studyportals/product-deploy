...
const compile = (opts) =>{
	...
	return new Promise((resolve, reject) =>{
		...
		const conditionSourceMaps = function(file){

			if(opts.uglify || !opts.sourcemaps){
		...
		gulp.src(opts.from, {base: opts.base})
			...
			.pipe(gulpif(conditionSourceMaps, sourcemaps.init())).on('error', reject)
			.pipe(gulpif(conditionBabel, js.babel.compile())).on('error', reject)
			.pipe(gulpif(conditionUglify, js.uglify.compile())).on('error', reject)
			.pipe(gulpif(conditionSourceMaps, sourcemaps.write('.'))).on('error', reject)
