...
class Deploy{
	 ...
	 * @param {String} [opts.cache_location='Core/Data/Cache/'] Location of the cache files.
	 */
	...
	clearCache(opts){
		...
		globby([path.join(opts.cache_location, '*'), '!.gitkeep'], opts.cwd)
	 ...
	 * @param {Object} [opts]
	 * @param {String} [opts.cwd=Deploy.opts.cwd] Current working directory
	 * @param {String} [opts.error_location='Core/Data/ErrorLog/'] Location of the cache files.
	 */
	...
	clearErrorLog(opts){
		...
		const defaults = {
			cwd: this.opts.cwd,
			error_location: 'Core/Data/Cache/'
		};
		opts = Object.assign(defaults, opts);

		log.info(`Clearing error logs...`);

		globby([path.join(opts.error_location, '*'), '!.gitkeep'], opts.cwd)
			.then(function then(paths) {
				paths.map(function map(item) {
					rimraf.sync(item);
				});
