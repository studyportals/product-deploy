...
class Watcher{
	...
	attachToGulp(gulp, buildDir, paths, options){
		...
		const args = process.argv.map(x=>x.toLowerCase()).join(',');
		if(args.indexOf(',wk') == -1 && args.indexOf(',wc') == -1){

			log.debug("Watchers active only on local deploys");
			// return;
		}
		...
		if(!global.logged){
			...
			log.debug(`Edit environmental variable PRTL_DISABLED_WATCHERS to disable specific watchers.`);
			log.debug(`Values in this variable are separated by ',' (css,js,etc)`);
		}

		if(!enabled){

			log.debug(`Is disabled. To enable, remove '${this.envName}' from PRTL_DISABLED_WATCHERS.`);
			return;
		}

		log.debug(`Is enabled. To disable, add '${this.envName}' to PRTL_DISABLED_WATCHERS.`);
		...
		gulp.on('stop', () =>{
			...
			sourceCollections.push({
				sources: paths.map(x=>{
					...
					return this._resolveGlob(x, cwd);
				}),
				base: cwd,
				origin: cwd
			});
	 ...
	 *
	 * @param message
	 * @deprecated replace by log.info / log.debug etc
	 */
	log(message){

		log.info(`${this.constructor.name}:`, message);
	}
