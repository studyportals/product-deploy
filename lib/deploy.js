		...
		const change = function(filePath){
			...
			switch(path.extname(filePath)){
				...
				case '.scss':
					...
					// TODO: get it once and update the references as we go
					return sass.getReferences()
						.then(ref =>{

							console.log(`Find dependencies for ${filePath}`);

							const dependencies = [];

							const addDepencenciesRecursive = (deps)=>{

								console.log('addDepencenciesRecursive', deps);
							...
							console.log('final deps', dependencies);

							compileSass(dependencies)
						})
						.catch(exc => { throw exc; });
