...
class Deploy{
	...
	/**
	 * Get a list of global ignores.
	 *
	 * @param {boolean} [glob=false] - Should we return the list formatted as globs?
	 * @returns {string[]}
	 * @private
	 */
	_getGlobalIgnores(glob){

		const globalIgnores = [
			path.resolve(this.opts.from, '.idea'),
			path.resolve(this.opts.from, '.git'),
			path.resolve(this.opts.from, 'dist'),
			path.resolve(this.opts.from, 'node_modules'),
			path.resolve(this.opts.from, 'vendor'),
			path.resolve(this.opts.from, 'bower_components'),
		];

		if(glob){

			return globalIgnores.map(ignore => {return `!${ignore}/**/*`});
		}
	...
	sass(from){
		...
		from = from || [
			`${this.opts.from}/**/*.scss`,
		].concat(this._getGlobalIgnores(true));
	...
	startWatchers(opts){
		...
		chokidar.watch(watch, {
			ignoreInitial: true,
			ignored: this._getGlobalIgnores().concat([
				function(string){
